...
module.exports = {
    "emails": {
        "user": (name = '', email = '', feedback = '', time = 0) => {
            let data = fs.readFileSync('./emails/user/index.html', 'utf-8')
            data_to_send = data.replace('%-%username%-%', name).replace('%-%usertime%-%', time).replace('%-%tablename%-%', name).replace('%-%useremail%-%', email).replace('%-%userfeedback%-%', feedback)
            return data_to_send;
        },
